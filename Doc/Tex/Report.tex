...
\subsubsection{Top-Level Folder Structure}
...
\subsubsection{Sub-Folder Structure}
...
Data within the \texttt{Raw}, \texttt{Utouched}, \texttt{QGis} and \texttt{Log}
folders are untracked by Git to avoid uploads of large data files into the
public domain.  To ensure that data can be shared between all contributors to
the ReMeltRadar project, a CSV based file tracking system is proposed.  Each
subfolder in the above directories should contain a \texttt{catalogue.csv} file
which lists the files contained within the folder. The CSV files will be tracked
by Git and hence allow for a version-controlled list of data files to be stored
in a central repository. There is some responsibility on the part of the project
contributors to modify the \texttt{catalogue.csv} files appropriately so that
their files are tracked and included for reference in the central repository.
However, tools are provided in \texttt{Src/data.py} to automatically update the
\texttt{catalogue.csv} files and provide reports on missing files.

\subsubsection{File Format for \texttt{catalogue.csv}}
The \texttt{catalogue.csv} file should be UTF-8 encoded and contain the headings
in Table \ref{TableDataCatalogueCSV} on the first line.  There should be one
entry per file with the values in Table \ref{TableDataCatalogueCSV}.  Each row
should contain at least \textbf{filename}, other fields are optional and can be
left blank.
...
\begin{table}[h]
  ...
  \begin{tabular}{l p{10cm}}
    ...
    \hline
    \textbf{filename} & name of the file to be catalogued. \\
    \textbf{size} & file size in bytes. \\
    \textbf{owner} & name of the person to collect the file from. \\
    \textbf{date\_added} & YYYY-MM-DD formatted date of the day the file was
    added \texttt{catalogue.csv}. \\
  \textbf{comment} & brief description displayed in the missing files report or for
reference purposes. \\
    \hline
  \end{tabular}
  \caption{Heading name and description of \texttt{catalogue.csv} file.}
  \label{TableDataCatalogueCSV}
\end{table}
...
\subsubsection{Generate Missing File Report}
To generate a missing file report, the following command should be called from
the project root using the option \texttt{-r}:
\begin{lstlisting}
python Src/data.py -r /path/from/root/to/data_folder 
\end{lstlisting}
which will generate a log file named \texttt{data\_report\_YYYYMMDD\_HHMMSS.log}
in \texttt{/Log} for \texttt{data\_folder}.
If you want do this for all subfolders, then you can use the option \texttt{-s}
\begin{lstlisting}
python Src/data.py -r -s /path/from/root/to/data_folder
\end{lstlisting}

\subsubsection{Automatically Update \texttt{catalogue.csv}}
To automatically update the \texttt{catalogue.csv} file in a given folder, the
following command should be called from the project root using the option
\texttt{-u}:
\begin{lstlisting}
  python Src/data.py -u /path/from/root/to/data_folder  
\end{lstlisting}
which will update the file \texttt{/path/from/root/to/data\_folder/catalogue.csv}
to include any files which are not currently included.  If you want to specify
the owner, then passing an additional argument will set the \texttt{owner} for
each record.
\begin{lstlisting}
  python Src/data.py -u /path/from/root/to/data_folder owner_name  
\end{lstlisting}
Similarly, to update catalogue files in add subfolders, use the \texttt{-s}
flag.
\begin{lstlisting}
  python Src/data.py -u -s /path/from/root/to/data_folder [owner_name] 
\end{lstlisting}
