...
def _get_gps_data(fn_gps, trace_nums):
    ...
    #Changed RD
    #Define length of line. Last
    kk=0 
    for line in lines[:-2]:
        kk=kk+1
        if kk>6:
            llinesplit = line.split(',',4)
            if (llinesplit[0].isdigit() and len(llinesplit)==5):
            #This was to catch if lines are only half stored. I thinkg
            #this happens only at the end. Hence don't go there.
            #if len(llinesplit[4][1:-1])==lengthllinesplit:
                gga.append(llinesplit[4][1:-1])
                ggis.append(llinesplit[0])
            else:
                continue
    if len(gga) == 0:
        raise ValueError('I can only do gga sentences right now')
