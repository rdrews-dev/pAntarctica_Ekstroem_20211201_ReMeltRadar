...
ROOT = '../../../../';
data_folder = fullfile(ROOT, 'Raw/ApRES/Rover/HF/Kinematic');

if ~exist(data_folder,'dir')
    error("Couldn't find the folder %s\nHas the data been downloaded?", data_folder)
end

prof = fmcw_load(fullfile(data_folder, '3_SubZero__181022.80_T1HR1H.dat'));
...
%%
subplot(1,2,1)
imagesc(1:Nfiles, rc, 20*log10(abs(flipud(data))).');
...
imagesc(1:Nfiles, rc, -(angle(flipud(data))).', 'AlphaData', flipud(power_norm).');
colorbar
